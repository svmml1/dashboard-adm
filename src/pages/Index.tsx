...
export default function Index() {
  const { sales, walletSummary, walletLoading } = useMainStore()

  const totalSales = sales.length
  const confirmed = sales.filter((s) => s.status === 'Pago').length
  const recentSales = sales.slice(0, 5)

  const saldoDisponivel = walletSummary?.saldoDisponivel ?? 0
  const totalRecebido = walletSummary?.totalRecebido ?? 0
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="hover:-translate-y-1 transition-transform duration-200">
            ...
            <CardContent>
              ...
              <p className="text-xs text-emerald-500 mt-1 font-medium">+12% em relação a ontem</p>
            </CardContent>
          </Card>
          <Card className="hover:-translate-y-1 transition-transform duration-200">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold font-mono">
                {walletLoading ? <span className="text-muted-foreground text-base">...</span> : formatCurrency(totalRecebido / 0.97)}
              </div>
              <p className="text-xs text-muted-foreground mt-1">Líquido: {formatCurrency(totalRecebido)}</p>
            </CardContent>
          </Card>
          ...
          <Card className="hover:-translate-y-1 transition-transform duration-200">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {Math.round((confirmed / totalSales) * 100)}% de conversão
              </p>
            </CardContent>
          </Card>
        </div>
                ...
                {recentSales.map((sale) => (
                  <div key={sale.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-9 w-9 rounded-full bg-primary/20 flex items-center justify-center shrink-0">
                        <span className="text-xs font-bold text-foreground">
                          {sale.competitor.substring(0, 2).toUpperCase()}
                        </span>
                      </div>
                    ...
                    <div className="text-right grid gap-0.5">
                      <p className="text-sm font-medium font-mono">{formatCurrency(sale.value)}</p>
